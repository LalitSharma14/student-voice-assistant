...
SOURCE_FILES = {
    ...
    "Maths": "class7_mathematics.json",
    "Sanskrit": "class7_sanskrit.json",
    "Science": "class7_science.json",
    "Social Science": "class7_social_science.json",
}
...
def load_subject_file(source_dir: Path, subject: str) -> dict:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Missing Class 7 source JSON: {path}")
...
def make_doc(subject: str, source: dict, chapter: dict, topic: dict) -> dict:
    ...
    return {
        ...
        "language": source.get("language") or chapter.get("language") or ("sa" if subject == "Sanskrit" else "en"),
        "status": "published",
...
def build_seed(source_dir: Path) -> list[dict]:
    docs = []
    seen = set()
    for subject in ["English", "Maths", "Sanskrit", "Science", "Social Science"]:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory containing the Class 7 English, Maths, Sanskrit, Science, and Social Science JSON files.",
    )
    parser.add_argument(
        "--output",
