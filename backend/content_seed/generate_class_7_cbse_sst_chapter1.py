...
DEFAULT_SOURCE_PATH = BASE_DIR / "class7_sst_chapter1.json"
OUTPUT_PATH = BASE_DIR / "class_7_cbse_sst_chapter1.json"
...
def content_id(subject: str, chapter: str, topic: str) -> str:
    return slug(f"class_7_CBSE_{subject}_{chapter}_{topic}")
...
def load_source(source_path: Path) -> dict:
    ...
    if isinstance(data, list):
        return {"chapters": data, "board": "CBSE", "subject": "Social Science", "class": 7}
    if isinstance(data, dict) and "chapters" in data:
        return data
    raise ValueError(f"{source_path} must contain a chapters array or an object with a chapters array.")


def make_doc(source: dict, chapter: dict, topic: dict) -> dict:
    ...
    if not chapter_title or not topic_title:
        raise ValueError(f"Missing chapter/topic title: {chapter}")
    ...
    book_name = str(source.get("bookName") or "Exploring Society: India and Beyond").strip()

    return {
        "id": content_id(subject, chapter_title, topic_title),
        "classLevel": "7",
        "board": str(source.get("board") or "CBSE").strip(),
        "subject": subject,
        ...
        "status": "published",
        "sourceLabel": f"Based on NCERT {book_name} + AI-generated teacher-style explanation",
        "studyContent": {
            "title": topic_title,
...
def build_seed(source_path: Path) -> list[dict]:
    source = load_source(source_path)
    docs = []
    seen = set()
    for chapter in source.get("chapters", []):
        for topic in chapter.get("topics", []):
            doc = make_doc(source, chapter, topic)
            if doc["id"] in seen:
...
def main():
    parser = argparse.ArgumentParser(description="Generate Class 7 CBSE SST chapter 1 content seed.")
    parser.add_argument("--source", default=str(DEFAULT_SOURCE_PATH), help="Class 7 SST source JSON path.")
    parser.add_argument("--output", default=str(OUTPUT_PATH), help="Output seed JSON path.")
    args = parser.parse_args()

    docs = build_seed(Path(args.source))
